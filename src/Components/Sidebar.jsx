...
export default function Sidebar() {
    ...
    return (
        <>
            <button className="md:hidden fixed top-4 left-4 z-50 p-2 rounded-md bg-blue-600 text-white dark:bg-blue-700 shadow-lg" onClick={toggleSidebar} aria-label="Toggle menu">
                {isOpen ? <XIcon className="h-6 w-6" /> : <MenuIcon className="h-6 w-6" />}
            </button>

            <div className="fixed top-0 left-0 w-1 h-full z-50 bg-gray-200 dark:bg-gray-800">
                <div className="bg-blue-600 dark:bg-blue-400 h-full transition-all duration-300 ease-out" style={{ height: `${scrollProgress}%` }}></div>
            </div>

            <aside className={`fixed top-0 left-0 z-40 h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-white shadow-xl transition-all duration-300 ease-in-out border-r border-gray-200 dark:border-gray-800 ${isOpen ? "w-64" : "-translate-x-full md:translate-x-0 md:w-64"}`}>
                            ...
                            {menuItems.map((item) => (
                                <li key={item.id}>
                                    <button
                                        ...
                                        className={`flex items-center w-full p-3 rounded-md transition-all duration-300 ${activeSection === item.id ? "bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 font-medium" : "hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300"}`}
